...
def transformer_model(input_tensor,attention_mask,
            embedding_table,masked_lm_positions,
            masked_lm_ids, masked_lm_weights,next_sentence_labels):
  ...
  #althrough the transformer_model in modeling.py does return all_layer_outputs 
  #but the loss caculation only use the last layer's output
  #all_layer_outputs = []
  for layer_idx in range(args.num_hidden_layers):
    ...
    """
    with scopes.ipu_shard(layer_idx + 2):
      with tf.variable_scope("layer_%d" % layer_idx):
        layer_input = prev_output
...
with ipu_scope("/device:IPU:0"):
    ...
    batch = ipu.ipu_compiler.compile(transformer_model, [input_tensor,
                                                attention_mask,
                                                embedding_table,
                                                masked_lm_positions,
                                                masked_lm_ids,
