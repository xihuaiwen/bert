...
class Dataset:
    def __init__(self,num_items):
        ...
        for i in range(0,self.num_items):
            ...
            _sequence_output = np.random.rand(args.batch_size,args.seq_length,args.hidden_size).astype(np.float16)
...
def bert_model(input_ids,input_mask,token_type_ids,
            ...
            masked_lm_weights,next_sentence_labels,sequence_output):
  ...
  return embedding_output,embedding_table,attention_mask,sequence_output,prev_output,train_op
...
with ipu_scope("/device:IPU:0"):
    sequence_output = tf.placeholder(tf.float16,shape=(args.batch_size, args.seq_length, args.hidden_size),name="sequence_output")
...
with tf.Session() as sess:
    ...
    for i in range(0,args.epochs):
        ...
        for (_input_ids,
            ...
            _masked_lm_weights,
            _next_sentence_labels,
            _sequence_output)  in data:
            embedding_output,embedding_table,attention_mask,seq_output,prev_output = sess.run(batch,feed_dict = {input_ids:_input_ids,
                                            ...
                                            sequence_output:_sequence_output})
